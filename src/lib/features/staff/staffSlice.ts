...
import { Staff, StaffState, PaginatedStaff, TrainerModule } from "./staffTypes";
...
const staffSlice = createSlice({
  ...
  reducers: {
    ...
    clearTrainersByCourse(state) {
      state.trainersByCourse = [];
    },
  },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<PaginatedStaff>) => {
          state.loading = false;
      ...
      .addCase(fetchStaffById.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Failed to fetch staff details";
      ...
      .addCase(createStaff.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Failed to create staff member";
      })
      ...
      .addCase(updateStaff.fulfilled, (state, action) => {
        ...
        if (action.payload.success) {
          ...
          const ddIndex = state.dropdownStaff.findIndex(
            (s) => s.staffId === updated.staffId
          );
          if (ddIndex !== -1) {
            state.dropdownStaff[ddIndex] = {
              ...state.dropdownStaff[ddIndex],
              ...updated,
            };
          }
      ...
      .addCase(updateStaff.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Failed to update staff member";
      })
      ...
      .addCase(deleteStaff.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Failed to delete staff member";
      })

      /* =====================================================
         ⭐ FETCH TRAINERS BY COURSE
        ===================================================== */
      .addCase(fetchTrainersByCourse.pending, (state) => {
        state.loading = true;
        state.error = null;
      })

      .addCase(
        fetchTrainersByCourse.fulfilled,
        (state, action: PayloadAction<TrainerModule[]>) => {
          state.loading = false;
          state.trainersByCourse = action.payload;
        }
      )

      .addCase(fetchTrainersByCourse.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Failed to fetch trainers by course";
        state.trainersByCourse = [];
      });
  },
});
