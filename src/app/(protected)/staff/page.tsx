...
const StaffList: React.FC = () => {
    ...
    const [sortModel, setSortModel] = React.useState<GridSortModel>([{ field: 'staffId', sort: 'asc' }]);
    ...
    const sortedStaff = staff.length > 0 ? [...staff].sort((a, b) => {
                ...
                <Box sx={{ height: 600, width: '100%', mb: 2 }}>
                    {error ? (
                        ...
                        <DataGrid
                            ...
                            disableColumnMenu
                            getRowId={(row) => row?.staffId ? row.staffId.toString() : Math.random().toString()}
                            loading={isLoading}
                            slots={{
                                noRowsOverlay: () => (
                            ...
                            <Box key={staffMember.staffId}>
                                ...
                                >
                                    ...
                                    <IconButton size="small">
                                        {expandedRows.includes(staffMember.staffId) ? <KeyboardArrowUp /> : <KeyboardArrowDown />}
                                    </IconButton>
                                </Box>

                                <Collapse in={expandedRows.includes(staffMember.staffId.toString())}>
                                    <CardContent>
                                        <Stack spacing={2}>
                                            <Box>
