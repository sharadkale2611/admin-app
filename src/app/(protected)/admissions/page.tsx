...
import { Admission, AdmissionStatus, EnrollmentType, PaymentStatus } from '@/lib/features/admission/admissionTypes';
...
const AdmissionList: React.FC = () => {
    ...
    const [sortModel, setSortModel] = React.useState<GridSortModel>([{ field: 'admissionId', sort: 'asc' }]);
    const [expandedRows, setExpandedRows] = React.useState<string[]>([]);
    const [localSearchTerm, setLocalSearchTerm] = React.useState(searchTerm);

    const { handleDelete } = useDeleteAdmission();

    const onDeleteAdmission = async (admissionId: number, studentName: string) => {
    ...
    const sortedAdmissions = admissions.length > 0 ? [...admissions].sort((a, b) => {
    ...
    const columns: GridColDef<Admission>[] = [
        ...
        {
            field: 'enrollmentType', headerName: 'Type', width: 130,
        ...
        },
        {
            ...
            renderCell: (params) => {
                ...
                else if (params.row.paymentStatus === PaymentStatus.Pending) color = 'warning';
                else if (params.row.paymentStatus === PaymentStatus.PartiallyPaid) color = 'error';

                return <Chip label={params.row.paymentStatus} size="small" color={color} />;
            }
        },
        ...
        { field: 'finalAmount', headerName: 'Final Amount', width: 120 },
        {
            ...
            renderCell: (params) => <Chip label={params.row.status ? 'Yes' : 'No'} size="small"
    ...
    return (
        <Box sx={{ p: isMobile ? 1 : 3 }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
                ...
                <Button variant="contained" startIcon={<Add />} component={Link} href="/admissions/create">
                    Add Admission
                </Button>
            </Stack>
            ...
            <Paper sx={{ mb: 3, p: 2 }}>
                <Stack direction={isMobile ? 'column' : 'row'} spacing={2} alignItems="flex-end">
                    <TextField
                        ...
                        InputProps={{
                            endAdornment: <IconButton onClick={handleSearchSubmit}><Search /></IconButton>
                        }}
                        size="small"
                        fullWidth={isMobile}
                    />
                    <FormControl size="small" sx={{ minWidth: 120 }}>
                        ...
                        <Select
                            ...
                            onChange={(e: SelectChangeEvent<string>) =>
                                handleStatusFilter(e.target.value === '' ? null : e.target.value as AdmissionStatus)
                            }
                        >
                    ...
                    <Button variant="outlined" size="small" onClick={handleResetFilters}>Reset</Button>
                </Stack>
            </Paper>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                ...
                <Typography>Page {page} of {totalPages} | Total: {admissions.length}</Typography>

                <Pagination count={totalPages} page={page} onChange={(_, newPage) => handlePageChange(newPage)} />
            </Box>
                            ...
                            <Collapse in={expandedRows.includes(adm.studentEnrollmentId.toString())}>
                                <CardContent>
                                    <Stack spacing={1}>
                                        ...
                                        <Chip label={`Payment: ${adm.paymentStatus}`} size="small" />
            ...
            {/* Bottom Pagination */}
            {totalPages > 1 && <Pagination count={totalPages} page={page} onChange={(_, newPage) => handlePageChange(newPage)} sx={{ mt: 3 }} />}
        </Box>
    );
};
